...
class SymmetricPositiveDefinite(Manifold):
    ...
    def barycenter(self, X : torch.Tensor, steps : int = 1, dim = 0, weights=None) -> torch.Tensor:
        """
        Compute several steps of the Kracher flow algorithm to estimate the 
        Barycenter on the manifold.
        """
        return functional.spd_mean_kracher_flow(X, None, maxiter=steps, dim=dim, weights=weights,return_dist=False)
