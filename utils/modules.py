...
class Graph_BiMap(nn.Module):
    ...
    def forward(self, X):
        ...
        if self.increase_dim:
            return self._bimap_multiplication(self.increase_dim(
                th.matmul(self._P, X.reshape((batch_size, channel_num, -1))).reshape(
                    (batch_size, channel_num, dim, dim))))
        else:
            return self._bimap_multiplication(th.matmul(self._P, X.reshape((batch_size, channel_num, -1))).reshape(
                (batch_size, channel_num, dim, dim)))
